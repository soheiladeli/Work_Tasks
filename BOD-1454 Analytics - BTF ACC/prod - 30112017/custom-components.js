...
//039	APPLINK(0, "Powerchart.exe", params);
...
schn.outorders.prototype.init = function (options) {
	...
	component.cclParams.push(this.getProperty("positionCd"));//043
	//alert('finished init');
...
//043  schn_newlookbacktable.send("^MINE^, "+pid+",^"+lookbackoptval+"^");
...
//023	& 030	& 041		
					...
					if (c_alert[i].ENCOUNTER_ID > 0) { //Checking encounter_ID as if component is visible on organiser-level MPage eg Dynamic Worklist, there is no encounter_ID
						targetHTML.push("<td class='schn-alerts-cell'><a class='schn-link' href='javascript:APPLINK(0,\"$APP_APPNAME$\",\"/PERSONID="+c_alert[i].PATIENT_ID+" /ENCNTRID="+c_alert[i].ENCOUNTER_ID+" /FIRSTTAB=^Acute Management Plans^\")' >" + c_alert[i].ANNOTATED + "</a></td>");
					} else {
				...
				} else if (c_alert[i].SOURCE_ID === "AL00134") {   // 045 BMI-for-age Alert
...
schn.dc_analytics.prototype.addEventHandlers = function() {
	...
	//049>> $("#"+compId+" .dc-selector").change(function(event){
	$("#"+compId+" .schn-filters-selector").change(function(event){	//<<049
...
schn.dc_analytics.prototype.render = function() {
	...
	//049>> targetHTML.push('<form><fieldset>',
	targetHTML.push('<div class="schn-filters-form"><form><fieldset>', //<<049
						'<div class="selection">',
						'<label for="schn-dc-lookback">Lookback Range: </label>',
						//049>> '<select name="lookback" id="schn-dc-lookback" size="1" class="dc-selector">',
						'<select name="lookback" id="schn-dc-lookback" size="1" class="schn-filters-selector">', //<<049
						...
						//049>> '<select name="facility" id="schn-dc-facility" size="1" class="dc-selector">',
						'<select name="facility" id="schn-dc-facility" size="1" class="schn-filters-selector">', //<<049
							'<option value="CHW" selected="selected">CHW</option>',
						'</select>',
						'</div>',
						'<div class="selection">',
						'<label for="schn-dc-wards">Ward: </label>',
						//049>> '<select name="wards" id="schn-dc-wards" size="1" class="dc-selector">',
						'<select name="wards" id="schn-dc-wards" size="1" class="schn-filters-selector">', //<<049
	...
	targetHTML.push('<div class="selection">',
						...
						//049>> '<select name="medserv" id="schn-dc-medserv" class="dc-selector">',
						'<select name="medserv" id="schn-dc-medserv" class="schn-filters-selector">', //<<049
	...
	targetHTML.push('<div class="selection">',
						...
						//049>> '<select name="amo" id="schn-dc-amo" class="dc-selector">',
						'<select name="amo" id="schn-dc-amo" class="schn-filters-selector">', //<<049
							'<option value="All">All</option>');
	...
	//049>> targetHTML.push('</fieldset></form>');
	targetHTML.push('</fieldset></form></div></section></div>'); //<<049
	targetHTML.push('<div id="dc-hover"></div>');
	//pie chart display
	//049>> targetHTML.push('<div id="dc-chart" class="dc-pie-chart" style="width:100%;height:250px"></div>');
	targetHTML.push('<div id="dc-chart" class="dc-pie-chart" style="width:100%;height:200px; margin-top:7px; margin-bottom:10px;"></div>'); //<<049
	
	//modal pop up window for on-click event
	targetHTML.push('<div id="schn-dc-modal" class="schn-modal">',
						//049>> '<div id="schn-dc-modal-content" class="schn-modal-content"><span class="schn-close">X</span>',
						'<div id="schn-dc-modal-content" class="schn-modal-content"><span class="schn-close"style="margin-bottom:10px";>&#10006;</span>', //<<049
							'<div id="dc-list">',
							'</div>',
						'</div>',
					'</div>');
...
schn.btf_acc.prototype.render = function() {
    ...
    //modal pop up window for on-click event
	//buiding 2 modal pop up windows lists for 2 diffrent ACC Status (CC and OVERDUE)lists
...
schn.clintrials.prototype.render = function() {
	...
	var acns0423_link = "acns0423.pdf";    //044
	...
	var GSUS3131090_link = "gs-us-313-1090.pdf";
	var highdosecyclovp16_link = "high_dose_cyclo_vp16.pdf";  //038
	...
	var kasper_link = "kaspers_et_al_jco_2013_flag_plus_liposomal_daunorubicin.pdf";    //42
	...
	if(trial.length > 0 ) {
		...
		for (i=0;i<trial.length;i++){
			...
			switch(sname){
				...
				// 046
				case "ALLR3":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + allr3_link + "\",\"\");'>ALLR3</a></td>");
					break;
				case "ANBL0032":
				...
				case "GS-US-313-1090":
					...
					break;
					//047
				case "HIGH DOSE CYCLO VP16":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + highdosecyclovp16_link + "\",\"\");'>High dose cyclo VP16 </a></td>");
					break;
...
// New mpage custom components for Summary visits 038start LV mod -17052017
/* ----------------------------------------------------------------------------------

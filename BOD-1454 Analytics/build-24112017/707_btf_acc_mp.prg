...
order by amo;prs.name_full_formatted, prs.person_id
head report
	; initialise variables to count returned records
	cnt = 0
	; allocate memory to the record structure
	stat = alterlist(enc->amofilter, 5)
head amo;prs.person_id
;;;; enc->qual[d1.seq].amo_person_id = prs.person_id
	...
	enc->amofilter[cnt]->amo_name = amo;prs.name_full_formatted
 
foot report
	stat = alterlist(enc->amofilter, cnt)
 
WITH NOCOUNTER, SEPARATOR=" ", FORMAT
SET size_array_amofilter = SIZE(enc->amofilter,5)

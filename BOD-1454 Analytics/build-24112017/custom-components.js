...
// 038 Tony Fitzsimons		14/07/17	Add more document hyperlinks for Oncology schn.clintrials AAML0531
// 039 Tony Fitzsimons		25/07/17	Update schn.alerts2 with hyperlink for AL00134
// 040 Tony Fitzsimons		08/08/17	Add anaesthetics management plan hyperlinks to schn.alerts2
// 041 Tony Fitzsimons		16/08/17	add kasper's to schn.clintrials
...
//170609mg	APPLINK(0, "Powerchart.exe", params);
	APPLINK(0, "$APP_AppName$", params);
}
...
schn.outorders.prototype.init = function (options) {
	...
	component.cclParams.push(this.getProperty("positionCd"));//042
	//alert('finished init');
...
schn.outorders.prototype.outorderlookbackopt = function (lookbackoptval, future_check) {
	...
	//alert(pid);
	//042
	var pocd = this.getProperty("positionCd");
...
//042  schn_newlookbacktable.send("^MINE^, "+pid+",^"+lookbackoptval+"^");
   schn_newlookbacktable.send("^MINE^, "+pid+",^"+lookbackoptval+"^,"+pocd);
};
...
//023	& 030	& 040		
				} else if ((c_alert[i].SOURCE_ID === "AL00109") || (c_alert[i].SOURCE_ID === "AL00128") || (c_alert[i].SOURCE_ID === "AL00129") || (c_alert[i].SOURCE_ID === "AL00130") || (c_alert[i].SOURCE_ID === "AL00135") || (c_alert[i].SOURCE_ID === "AL00136")){   //Kids GPS or neuro/seizure plans.		
...
//170609mg				targetHTML.push("<td class='schn-alerts-cell'><a class='schn-link' href='javascript:APPLINK(0,\"Powerchart.exe\",\"/PERSONID="+c_alert[i].PATIENT_ID+" /ENCNTRID="+c_alert[i].ENCOUNTER_ID+" /FIRSTTAB=^Acute Management Plans New^\")' >" + c_alert[i].ANNOTATED + "</a></td>");
						targetHTML.push("<td class='schn-alerts-cell'><a class='schn-link' href='javascript:APPLINK(0,\"$APP_AppName$\",\"/PERSONID="+c_alert[i].PATIENT_ID+" /ENCNTRID="+c_alert[i].ENCOUNTER_ID+" /FIRSTTAB=^Acute Management Plans New^\")' >" + c_alert[i].ANNOTATED + "</a></td>");
					} else {
						targetHTML.push("<td class='schn-alerts-cell'>" + c_alert[i].ANNOTATED + " - see Management Plans</td>");
					}
...
//039				
				} else if (c_alert[i].SOURCE_ID === "AL00134") {   //BMI-for-age Alert
					targetHTML.push("<span class='schn-alerts-cell'><a class='schn-link' href='javascript:APPLINK(100, \"http://chw.schn.health.nsw.gov.au/ou/weight_management/resources/clinical_pathways/schn_weight_management_pathway.pdf\",\"\");' >" + c_alert[i].ANNOTATED + "</a></span>");
...
SCHN BTF Altered Caling Criteria (ACC)
...
schn.btf_acc.prototype.addEventHandlers = function() {
    ...
    $("#" + compId + " .schn-filters-selector").change(function(event) {
        //alert("Changed a filter");
		if(event.target.id == 'schn-accc-amo'){
			$('#schn-dc-myOwn').removeAttr('checked');
		}
        $("#acc-hover").html("");
        component.updateFilters();
...
schn.btf_acc.prototype.render = function() {
	...
	targetHTML.push('<input type="checkbox" id="show_filters" role="button">');
	targetHTML.push('<label for="show_filters" onclick=""><span>Change BTF ACC Filters &#9776;&#9662;</span><span style="font-weight: bold;">&#10005;</span></label>');
	...
	//pie chart display
    targetHTML.push('<div id="acc-chart" class="acc-pie-chart" style="width:100%;height:200px; margin-top:30px; margin-bottom:20px;"></div>');

    //modal pop up window for on-click event
	//buiding 3 modal pop up windows lists for 3 diffrent ACC Statuse (ACC, OVERDUE and NRR)
	...
	targetHTML.push('<div id="schn-acc-modal2" class="schn-modal">',
						...
						'</div>',
					'</div>');
	targetHTML.push('<div id="schn-acc-modal3" class="schn-modal">',
						'<div id="schn-acc-modal-content" class="schn-modal-content">',
						'	<span class="schn-close" style="margin-bottom: 10px;">&#10006;</span>',
							'<div id="acc-list3">',
							'</div>',
	...
	acc_modal3 = document.getElementById('schn-acc-modal3');
...
schn.btf_acc.prototype.countArray = function(summs) {
    ...
    //3 Modal window ACC lists for 3 different ACC types (ACC, OVERDUE and NRR)
	var acc_list = document.getElementById('acc-list');
	var acc_list2 = document.getElementById('acc-list2');
	var acc_list3 = document.getElementById('acc-list3');
    pieHTML = [];
	pieHTML2 = [];
	pieHTML3 = [];

	//Headers for 3 ACC type modal windows
	pieHTML.push("<h4 class='boldie'>BTF ACC Records for Specialty: " + medLook + ", Ward: " + wardLook + ", Encounter Type: " + encntrtypeLook + "</h4>");
	pieHTML2.push("<h4 class='boldie'>BTF ACC Overdue Records for Specialty: " + medLook + ", Ward: " + wardLook + ", Encounter Type: " + encntrtypeLook + "</h4>");
	pieHTML3.push("<h4 class='boldie'>BTF ACC NRR Records for Specialty: " + medLook + ", Ward: " + wardLook + ", Encounter Type: " + encntrtypeLook + "</h4>");
	...
	if (summs.length > 0) {
        ...
        pieHTML2.push('</tr>');
		pieHTML3.push('<table id= "incomplete-dc">');
		pieHTML3.push('<tr><th>Patient Name</th><th>MRN</th><th>Ward</th><th>Next Review</th><th>Last Updated Dt</th><th>Attending Dr.</th><th>Last Signed Clinician</th><th>Specialty</th><th>ACC Comment</th><th>General Comment</th>');
        pieHTML3.push('</tr>');

		//looping through all ACC records and push ACC types to 3 different Modal lists
		for (j = 0; j < summs.length; j++) {
			...
			} else if (summs[j].ACC_STATUS == "NRR"){
				pieHTML3.push('<tr>');
                pieHTML3.push('<td><a class="schn-link" href="javascript:schn.openChart(' + summs[j].PERSON_ID + ', ' + summs[j].ENCNTR_ID + ');">' + summs[j].NAME + '</a></td>');
                pieHTML3.push('<td>' + summs[j].MRN + '</td>',
							  '<td>' + summs[j].WARD.trim() + '</td>',
							  '<td>' + summs[j].NEXT_REVIEW + '</td>',
							  '<td>' + summs[j].LAST_UPDATED + '</td>',
							  '<td>' + summs[j].AMO + '</td>',
	...
	//if no ACC records
		...
		pieHTML.push("<p>No BTF ACC Documnts have been recorded for ward: <span class='boldie'>" + wardLook + "</span>, Encounter type: <span class='boldie'>" + encntrtypeLook + "</span>, Specialty: <span class='boldie'>" + medLook + "</span>.</p>");
	}
	// end if

    pieHTML = pieHTML.join("");
	pieHTML2 = pieHTML2.join("");
	pieHTML3 = pieHTML3.join("");

	//publishing all 3 ACC lists
	acc_list.innerHTML = (pieHTML);
    acc_list2.innerHTML = (pieHTML2);
	acc_list3.innerHTML = (pieHTML3);
	...
	var data = [{
        ...
        label: "OVERDUE",//"In Progress",
        data: overdue,//inProgress,
        color: "#7D0096"
    }, {
        label: "NRR",//"Not Commenced",
        data: nrr,//notCommenced,
        color: "#DE000F"
    }]
    ...
    function pieClick(event, pos, obj) {
		...
		if (obj) {
            ...
            } else if (obj.series.label == "NRR") {
				acc_modal3.style.display = "block";
            }
        }
    }
	//end pie chart data
...
schn.btf_acc.prototype.updateFilters = function() {
	...
	if (filterRange.length == 0) {
        $("#acc-chart").html("No results for ward: <span class='boldie'>" + wardLook + "</span>, Encounter Type: <span class='boldie'>" + encntrtypeLook + "</span>, Specialty: <span class='boldie'>" + medLook + "</span>");
        $("#acc-list").html("");
	} else {
...
schn.clintrials.prototype.render = function() {
	...
	var aaml0531_link = "aaml_0531.pdf";
	var aaml1031_link = "aaml1031doc.pdf";
	var aaml1531_link = "aaml1531doc.pdf";  //025
	var acns0334_link = "acns0334_protocol_amd_3_28jan2016.pdf";
	var acns0423_link = "acns0423.pdf";
	...
	var anhl0221_link = "anhl0221.pdf";
	...
	var aren0533_link = "aren0533doc.pdf";  //029
	var aren0534_link = "aren0534doc.pdf";  //031
	...
	var cog99703_link = "cog_99703.pdf"; // 34
	var colorectalFox6_link = "colorectal_metastatic_folfox6.pdf";
	var denosumab_link = "denosumab_20062004_amend7protocol_15may13.pdf";
	var ecrestamendment5_link = "ecrest_protocol_am5_17oct16.pdf";  //028
	var ecreststorageadmmanual_link = "ecrest_storage_and_administration_manual_14dec16_clean.pdf";  //028
	var esphall_link = "esphall_protocol_amd_01_1_dec_2009.pdf";
	var fablmb96_link = "fab_lmb96_ccg-5961_amd_6.pdf";
	var GSUS3131090_link = "gs-us-313-1090.pdf"; //045
	var intreallsr_link = "intreall_sr_2010_protocol_version_1_8_1_nov_2012.pdf"
	var intreallsr2010appendixv32_link = "intreall_sr_2010_anzchog_appendix_version_3_2_august_2016.pdf";  //028
	var intreallsr2010leures_link = "intreall_sr_2010__leucovorin_rescue.pdf";  //028
	var intreallsr2010v19mar2016_link = "intreall_sr_2010_protocol_amendment_v1_9.pdf";  //031
	var joe_chemo_link = "joe_chemotherapy_for_retinoblastoma.pdf";
	var kasper_link = "kaspers_et_al_jco_2013_flag_plus_liposomal_daunorubicin.pdf";
	var lchiv_link = "lch-iv_protocol_amended_version_nov_2014_international_approved_april_2015.pdf";
	var lowgradegliomaavastin_link = "avastin_manas_paper.pdf";  //028
	...
	var siopenamendment4jul2009_link = "siopen_amendment_4_hrnbl1_protocol_2009.pdf";  //032
	var siopenv17_link = "siopen_v1_7_protocol_dated_04apr2014.pdf";  //032
	...
	var taclt2011002_link = "t2011-002_aza_protocol_am1_clean_1-20-2014.pdf";  //028
		...
		for (i=0;i<trial.length;i++){
			...
			switch(sname){
				...
				case "AALL1231":
					...
					break
				case "AAML0531":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + aaml0531_link + "\",\"\");'>AAML0531</a></td>");
					break;
				case "AAML1031":
				...
				//043
				case "ACNS0423":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + acns0423_link + "\",\"\");'>ACNS0423</a></td>");
					break;
				case "ACNS0831":
				...
				case "ANBL1232":
					...
					break;
				case "ANHL0221":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + anhl0221_link + "\",\"\");'>ANHL0221</a></td>");
					break;
				case "ANHL1131":
				...
				//029
				case "AREN0533":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + aren0533_link + "\",\"\");'>AREN0533</a></td>");
					break;	
				//031
				case "AREN0534":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + aren0534_link + "\",\"\");'>AREN0534</a></td>");
					break;
				case "ARST0431":
				...
				case "BLINATUMOMAB":
					...
					//034
				case "COG 99703":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + cog99703_link + "\",\"\");'>COG 99703</a></td>");
					break;		
				...
				case "DENOSUMAB":
					...
					break;
				case "ECREST AMENDMENT 5":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + ecrestamendment5_link + "\",\"\");'>ECREST Amendment 5</a></td>");
					break;
				case "ECREST STORAGE & ADMINISTRATION MANUAL":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + ecreststorageadmmanual_link + "\",\"\");'>ECREST Storage & Administration Manual</a></td>");
					break;
				case "ESPHALL":
				...
				case "FABLMB96":
					...
					break;
				case "GS-US-313-1090":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + GSUS3131090_link + "\",\"\");'>GS-US-313-1090</a></td>");
					break;
					//045
				case "INTREALL SR 2010":
					...
					break;
				case "INTREALL SR 2010 APPENDIX V3.2":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + intreallsr2010appendixv32_link + "\",\"\");'>IntReALL SR 2010 Appendix v3.2</a></td>");
					break;
				case "INTREALL SR 2010 LEUCOVORIN RESCUE":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + intreallsr2010leures_link + "\",\"\");'>IntReALL SR 2010 Leucovorin rescue</a></td>");
					break;
				//031
				case "INTREALL SR 2010 V1.9 MARCH2016":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + intreallsr2010v19mar2016_link + "\",\"\");'>IntreALL SR 2010 v1.9 March2016</a></td>");
					break;
				case "JOE CHEMOTHERAPY FOR RETINOBLASTOMA":
					...
					break;	
				//041
				case "KASPERS ET AL JCO 2013 FLAG PLUS LIPOSOMAL DAUNORUBICIN":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + kasper_link + "\",\"\");'>Kaspers et al JCO 2013 FLAG plus Liposomal Daunorubicin</a></td>");
					break;
				case "LCH-IV":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + lchiv_link + "\",\"\");'>LCH-IV</a></td>");
					break;					
				case "LOW-GRADE GLIOMA AVASTIN PROTOCOL":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + lowgradegliomaavastin_link + "\",\"\");'>Low-grade Glioma Avastin protocol</a></td>");
					break;
				case "MEK116540":
				...
				case "SIOPEL 4":
					...
					break;
				//032
				case "SIOPEN AMENDMENT 4 JUL 2009":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + siopenamendment4jul2009_link + "\",\"\");'>SIOPEN Amendment 4 Jul 2009</a></td>");
					break;
				//032
				case "SIOPEN V1.7":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + siopenv17_link + "\",\"\");'>SIOPEN v1.7</a></td>");
					break;
				case "SJMB12":
				...
				//028
				case "TACL T2011-002":
					targetHTML.push("<td><a class='schn-link' href='javascript:APPLINK(100, " + ref_doc_path + taclt2011002_link + "\",\"\");'>TACL T2011-002</a></td>");
					break;
				default:
...
// end schn.documents <-033


/********************************************************************************
//  035   OneView Communication component
/********************************************************************************/
...
schn.oneview_connect.prototype.render = function() {
	...
	//do something here with the targetHTML (component.data will have data type defined by this.cclDataType)
	targetHTML.push('<div class="content-body oneview_component_body">');
	
	
	//var oneview_url = 'EmployeeID=' + oneviews[0].EMPLOYEEID + '&StaffFirstName=' + oneviews[0].STAFFFIRSTNAME + '&StaffLastName=' + oneviews[0].STAFFLASTNAME + '( ' + oneviews[0].STAFFDESIGNATION + ')&StaffUsername=' + oneviews[0].STAFFUSERNAME + '&MRN=' + oneviews[0].MRN + '&FacilityID=' + oneviews[0].FACILITYID;
	var oneview_url = 'EmployeeID=' + oneviews[0].STAFFUSERNAME + '&StaffFirstName=' + oneviews[0].STAFFFIRSTNAME + '&StaffLastName=' + oneviews[0].STAFFLASTNAME + '( ' + oneviews[0].STAFFDESIGNATION + ')&StaffUsername=' + oneviews[0].STAFFUSERNAME + '&MRN=' + oneviews[0].MRN + '&FacilityID=' + oneviews[0].FACILITYID;
	...
	targetHTML.push('<div id="oneview_iframe"><iframe id="oneview_app" src="http://mgt-test01.ov.schn.health.nsw.gov.au/Oneview.ClinicalCommunicationsComponent.Web_deploy/?' + oneview_url + '" width="100%"></iframe></div>');
...
/*	,'RegDate3="',patrecord[0].ENCID[2].PREV_REGDT," , ",'"'
						...
						//	'DischargeSum3= "',patrecord[0].ENCID[3].DISUM_EVENT_ID,'"',
							...
							*/
							//); 						
/* ----------------------------------------------------------------------------------
SCHN PATIENT CONTACTS COMPONENT - COMPONENT FOR NOK DETAILS
------------------------------------------------------------------------------------*/
/*Put name of component here*/ 
